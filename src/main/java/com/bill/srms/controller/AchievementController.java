...
public class AchievementController {
    ...
    public RespBean add(Achievement achievement){
        ...
        try {
            ...
            return RespBean.unprocessable("成果创建失败"+e.getMessage(), achievement);
        }
        if (result)
            return RespBean.ok("成果创建成功", achievement);
        return RespBean.unprocessable("成果创建失败", achievement);
    }
    ...
    public RespBean get(@PathVariable Integer id){
        ...
        if (achievement!=null)
            return RespBean.ok("获取成果",achievement);
        return RespBean.unprocessable("该成果不存在");
    }
